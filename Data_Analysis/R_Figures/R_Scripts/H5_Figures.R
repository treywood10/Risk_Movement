...
# Import dataset #
dat_bd <- read_dta("data/RR_FC_bd.dta")

# Betas #
betas <- as.matrix(read.table("betas/betas_RR_FC_bd.txt", header = T))

# var-cov #
vcov <- as.matrix(read.table("vcovs/vcovs_RR_FC_bd.txt", header = T))
...
x.of.i <- seq(from = 0, to = 45, by = 0.01)
...
rep_lag_best <- mean(dat_bd$lag_best, na.rm = TRUE)
...
set.x <- cbind(rep_lag_risk_ratio, x.of.i, rep_lag_best, rep_best_time, rep_lag_nlights_mean, rep_lag_drought_prop,
               rep_lag_mountains_prop, rep_lag_dist_unit, rep_lag_dist_border_own, rep_lag_dist_cap_hun, rep_lag_days_to_urban, 
               rep_lag_hq, rep_lag_zone_de_confidence, rep_lag_neigh_troops, rep_lag_quality, 
               rep_lag_no_troops, 1)
...
plot_bd <- ggplot(data = plot_RR_bd) +
  ...
  geom_rug(data = dat_bd, aes(x = lag_duration, y = -1), sides = "b", 
           position = position_jitter(w = 0.3, h = 0), alpha = 0.15) +
  ylim(c(-1, 1)) +
  xlab(expression("FC Duration")) +
...
ggsave("Graphs/gg_H5_bd.jpg", width = 14, height = 12, dpi = 400)
...
# Import dataset #
dat_OSV <- read_dta("data/RR_FC_OSV.dta")

# Betas #
betas <- as.matrix(read.table("betas/betas_RR_FC_OSV.txt", header = T))

# var-cov #
vcov <- as.matrix(read.table("vcovs/vcovs_RR_FC_OSV.txt", header = T))
...
plot_OSV <- ggplot(data = plot_RR_OSV) +
  ...
  geom_rug(data = dat_OSV, aes(x = lag_duration, y = -2), sides = "b", 
           position = position_jitter(w = 0.3, h = 0), alpha = 0.15) +
  xlab(expression("FC Duration")) +
  ylab(expression("Predicted Number of Troops in Cell")) +
  ylim(c(-1,1)) + 
  ...
  theme(plot.title = element_text(hjust = 0.5, size = 35),
        ...
        axis.ticks.length = unit(10, "pt"),
        text = element_text(family = "Times New Roman")) 
ggsave("Graphs/gg_H5_OSV.jpg", width = 14, height = 12, dpi = 400)
...
# Import dataset #
dat_Reb <- read_dta("data/RR_FC_Rebs.dta")

# Betas #
betas <- as.matrix(read.table("betas/betas_RR_FC_Rebs.txt", header = T))

# var-cov #
vcov <- as.matrix(read.table("vcovs/vcovs_RR_FC_Rebs.txt", header = T))
...
plot_Rebs <- ggplot(data = plot_RR_Rebs) +
  ...
  geom_rug(data = dat_Reb, aes(x = lag_duration, y = -2), sides = "b", 
           position = position_jitter(w = 0.3, h = 0), alpha = 0.15) +
  xlab(expression("FC Duration")) +
  ylab(expression("Predicted Number of Troops in Cell")) +
  ylim(c(-1, 1)) +
  ...
  theme(plot.title = element_text(hjust = 0.5, size = 35),
        ...
        axis.ticks.length = unit(10, "pt"),
        text = element_text(family = "Times New Roman")) 
ggsave("Graphs/gg_H5_Rebs.jpg", width = 14, height = 12, dpi = 400)
...
# Import dataset #
dat_Gov <- read_dta("data/RR_FC_Gov.dta")

# Betas #
betas <- as.matrix(read.table("betas/betas_RR_FC_Gov.txt", header = T))

# var-cov #
vcov <- as.matrix(read.table("vcovs/vcovs_RR_FC_Gov.txt", header = T))
...
# Create matrix of representative values and the incremental change in X #
set.x <- cbind(rep_lag_risk_ratio, x.of.i, rep_lag_best, rep_best_time, rep_lag_nlights_mean, rep_lag_drought_prop,
               rep_lag_mountains_prop, rep_lag_dist_unit, rep_lag_dist_border_own, rep_lag_dist_cap_hun, rep_lag_days_to_urban, rep_lag_hq,
               rep_lag_zone_de_confidence, rep_lag_neigh_troops, rep_lag_quality, rep_lag_no_troops, 1)
...
plot_Gov <- ggplot(data = plot_RR_Gov) +
  ...
  geom_rug(data = dat_Gov, aes(x = lag_duration, y = 0), sides = "b", 
           position = position_jitter(w = 0.3, h = 0), alpha = 0.15) +
  xlab(expression("FC Duration")) +
  ylab(expression("Predicted Number of Troops in Cell")) +
  ylim(c(-1, 1)) + 
  ...
  theme(plot.title = element_text(hjust = 0.5, size = 35),
        ...
        axis.ticks.length = unit(10, "pt"),
        text = element_text(family = "Times New Roman")) 
ggsave("Graphs/gg_H5_Gov.jpg", width = 14, height = 12, dpi = 400)
...
best <- as_tibble(dat_bd$lag_duration)
best <- best %>%
  rename(duration = value) %>%
  mutate(group = "Battle Deaths")


OSV <- as_tibble(dat_OSV$lag_duration)
OSV <- OSV %>%
  rename(duration = value) %>%
  mutate(group  = "All OSV")
